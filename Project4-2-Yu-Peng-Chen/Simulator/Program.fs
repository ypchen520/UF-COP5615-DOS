...
let main argv =
    try 
        ...
        let numShareKey: int = numberOfUser
        ...
        let mutable requestNum: int = (numberOfUser * 2) + numShareKey + numTweet + numMention + numHashtag + (numDisconnect * 2)
        ...
        | "user" -> 
            let userID = 0
            printfn "You are in the user mode"
            printfn "Here are the options for a client:\n1. register\n2. "
            // while true do
            //     let userOption = Console.ReadLine() |> ignore
        ...
        | "testing" ->
            ...
            while true do
                if not flagRegister then
                    ...
                    flagRegister <- true
                if not flagFirstConnect then
                    simulator <! SimFirstConnect numberOfUser
                    Threading.Thread.Sleep(waitTime)
                    flagFirstConnect <- true
                if not flagShareKey then
                    simulator <! ShareKey numShareKey
                    Threading.Thread.Sleep(waitTime)
                    flagShareKey <- true
                if not flagSubscribe then
                    for i = 0 to numCelebrity-1 do
                        simulator <! SimSubscribe numFollower
                        Threading.Thread.Sleep(waitTime)
                    flagSubscribe <- true
                if not flagDisconnect then    
                    simulator <! SimDisconnect numDisconnect
                    Threading.Thread.Sleep(waitTime)
                    flagDisconnect <- true
                ...
                if not flagConnect then    
                    simulator <! SimConnect numDisconnect
                    Threading.Thread.Sleep(waitTime)
                    flagConnect <- true
                ...
                if not flagQHashTag then    
                    simulator <! SimQueryHashtag numQHashtag
                    Threading.Thread.Sleep(waitTime)
                    flagQHashTag <- true
                if not flagQMention then    
                    simulator <! SimQueryMentioned numQMention
                    Threading.Thread.Sleep(waitTime)
                    flagQMention <- true
                if not flagRetweet then    
                    simulator <! SimRetweet numRetweet
                    Threading.Thread.Sleep(waitTime)
                    flagRetweet <- true
                if not flagQSubscribe then    
                    simulator <! SimQuerySubscribed numQSubscribe
                    Threading.Thread.Sleep(waitTime)
                    flagQSubscribe <- true
        ...
        | "zipf" ->
            ...
            while true do
                ...
                if not flagSubscribe then
                    for i = 0 to numCelebrity-1 do
                        simulator <! SimSubscribe zipfDist.[i]
                        // printfn "number of follower: %A" zipfDist.[i]
                        Threading.Thread.Sleep(waitTime)
                    flagSubscribe <- true
                if not flagDisconnect then    
